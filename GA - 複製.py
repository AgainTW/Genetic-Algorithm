...
pool_path = []
pool_size = 8
evolu_times = [1, 5, 10, 20, 30, 50, 100]
cut = [ len(input)//2, len(input)//3, (len(input)//3)*2]
prob = [0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5]

aaa = []	#分數
bbb = []	#路徑
for i in cut:
	for j in prob:
		for k in evolu_times:
			genetic_pool = []
			pool_score = []
			pool_path = []
			...
			# 演化
			genetic_pool = evolution(genetic_pool,i,j)
			for l in range(k-1):
				genetic_pool = evolution(genetic_pool,i,j)

			# 解碼並計算多輪演化後分數
			for l in range(len(genetic_pool)):
				pool_score.append([ score( path_gen( genetic_pool[i][:], len(genetic_pool[0][:]) ) ) ,i ])
			pool_score = mergeSort(pool_score)
			for l in range(len(genetic_pool)):
				pool_path.append( path_gen( genetic_pool[pool_score[i][1]][:], len(genetic_pool[0][:]) ) )	
...
filename = '分析.csv'
with open('分析.csv', 'w', newline='') as csvfile:
	# 建立 CSV 檔寫入器
	writer = csv.writer(csvfile)
	writer.writerow([" 交配節點 ", "突變機率", "演化時間", "分數", "路徑"])

	w_count = 0
	# 寫入列資料
	for i in range(len(cut)):
		for j in range(len(prob)):
			for k in range(len(evolu_times)):
				writer.writerow([cut[i], prob[j], evolu_times[k], aaa[w_count], bbb[w_count]])
				w_count += 1

'''
print("yourAssignment",final_path)
print("Cost",pool_score[-1][0])
'''